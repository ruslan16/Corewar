...
/*   Updated: 2020/03/15 15:32:48 by lclay            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	oct_print_spaces(t_struct *f, int n)
{
	...
	if (f->minus == false)
	{
		...
		else
		{
			...
			len = (f->precision == 0 && n == 0 && f->hash == false) ? 0 : len;
			if (f->width > len)
				space_count += (f->width - len);
		}
	}
...
void	oct_print_hash(t_struct *f, int n)
{
	if (f->hash == true)
	{
		if (n != 0)
			f->hash_flag = true;
		else if (f->prec_flag == true || f->zero == true)
			f->hash_flag = true;
	}
	if (f->hash_flag == true)
		f->nprinted += write_zero(1);
}
